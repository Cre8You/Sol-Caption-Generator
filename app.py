...
def get_x_bearer_token() -> Optional[str]:
    try:
        return st.secrets["X_BEARER_TOKEN"]
    except Exception:
        return os.getenv("X_BEARER_TOKEN")


@st.cache_data(ttl=300)
def fetch_x_trends(woeid: str, bearer_token: str) -> List[Dict[str, str]]:
    ...
    for item in data_items:
        name = item.get("trend_name") or item.get("name") or item.get("query") or "トレンド名不明"
        volume = str(item.get("post_volume") or item.get("tweet_volume") or "件数不明")
        ...
        trends.append(
            {
                ...
                "name": name,
                "volume": volume,
                "angle": "X API から取得したトレンドです。投稿前に内容との自然な相性を確認してください。",
                "suggestedTheme": f"{name}から着想した街歩きの投稿",
...
def build_caption(
    ...
    if post_type == "街ブラ":
        intro = (
            f"いつもの街を歩いていたら、{theme}にふっと出会えました{shoes}"
            if platform == "X"
            else f"いつもの街をゆっくり歩いていたら、{theme}に出会えた日{shoes}"
        )
    elif post_type == "写真":
        intro = (
            f"{theme}を切り取った一枚です。眺めていた空気まで写っていた気がします{camera}{sparkle}"
            if platform == "X"
            else f"{theme}を切り取った一枚です。見返すたび、そのときの空気まで思い出します{camera}{sparkle}"
        )
    elif post_type == "イベント告知":
        intro = (
            f"{theme}のお知らせです。気になっていた方に届いたらうれしいです{megaphone}{star}"
            if platform == "X"
            else f"{theme}のお知らせです。参加を考えている方に、やさしく届きますように{megaphone}{star}"
        )
    elif post_type == "出演者募集":
        intro = (
            f"{theme}の参加者を募集しています。表現が好きな方に届いたらうれしいです{megaphone}{heart}"
            if platform == "X"
            else f"{theme}の参加者を募集しています。ことばや表現が好きな方と出会えたらうれしいです{megaphone}{heart}"
        )
    else:
        intro = (
            f"{theme}について、毎日の中で取り入れやすい形でまとめてみました{leaf}{sparkle}"
            if platform == "X"
            else f"{theme}について、毎日の中で無理なく取り入れやすい形でまとめました{leaf}{sparkle}"
        )

    if location:
        location_sentence = (
            f"{pin} 場所は{location}。歩いているだけで気持ちがほどけるような空気でした{sun}"
            if platform == "X"
            else f"{pin} 場所は{location}。景色だけじゃなく、その場の温度まで残しておきたくなりました{sun}"
        )
    else:
        location_sentence = ""
    ...
    if deadline:
        deadline_sentence = (
            f"{clock} 締切は{deadline}です。気になる方はどうぞお早めに{sparkle}"
            if platform == "X"
            else f"{clock} 応募や参加の締切は{deadline}です。タイミングが合う方はぜひチェックしてみてください{sparkle}"
        )
    else:
        deadline_sentence = ""
...
def run_self_tests() -> None:
    ...
    fake_trends = SAMPLE_TRENDS["東京"]
    assert len(fake_trends) > 0
    assert "name" in fake_trends[0]
...
with right:
    ...
    with tab3:
        with st.container(border=True):
            st.markdown("#### Xトレンド候補")
            st.caption("X APIの Bearer Token があれば実データ取得、なければサンプル表示に切り替わります。")
            ...
            with trend_col2:
                default_woeid = DEFAULT_WOEID_MAP.get(region, "1118370")
                woeid = st.text_input("WOEID", value=default_woeid, key="trend_woeid")

            trend_keyword = st.text_input("絞り込み", placeholder="例: 写真 / 街歩き / カフェ", key="trend_keyword")

            st.info("トレンドを見つけたら、そのままテーマとキーワードに反映できます。自動投稿ではなく【候補を見て選ぶ】流れにしてあるので、無理にトレンドへ乗りすぎない設計です。")

            bearer_token = get_x_bearer_token()
            use_live_data = st.toggle("X API の実データを使う", value=bool(bearer_token), disabled=not bool(bearer_token))

            trend_results: List[Dict[str, str]]
            if use_live_data and bearer_token:
                try:
                    trend_results = fetch_x_trends(woeid=woeid, bearer_token=bearer_token)
                    st.success("X API から最新トレンドを取得しました。")
                except requests.HTTPError as e:
                    st.error(f"X API の取得でエラーが出ました: {e}")
                    trend_results = SAMPLE_TRENDS[region]
                except Exception as e:
                    st.error(f"トレンド取得に失敗しました: {e}")
                    trend_results = SAMPLE_TRENDS[region]
            else:
                trend_results = SAMPLE_TRENDS[region]
                if not bearer_token:
                    st.warning("X_BEARER_TOKEN が未設定のため、サンプルデータを表示しています。")
...
st.divider()
st.markdown("### X API 接続の手順")
st.markdown(
    "1. X Developer Platform で App を作成して Bearer Token を取得する\n"
    "2. Streamlit Community Cloud の Secrets に `X_BEARER_TOKEN = \"...\"` を登録する\n"
    "3. `requirements.txt` に `requests` を追加する\n"
    "4. アプリを再デプロイする\n"
    "5. トレンド取得タブで『X API の実データを使う』をオンにする"
)
